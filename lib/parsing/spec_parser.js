...
SpecParser.prototype = {
    ...
    parseTableDesc: function (name, desc) {
        ...
        var return_spec = {};
        return_spec[name.split('.')[1]] = s._cleanEmptyProperty({
            description: name,
    ...
    _parseType: function (col) {
        ...
        switch (mysqlType) {
            ...
            case 'VARCHAR':
            case 'CHAR':
                if( col.Default && col.Default.startsWith("Array:#") ) {
                    var items_ref = col.Default.split(':')[1];
                    items_ref = items_ref.replace('#/','#/definitions/');
                    return {
                        type:'array',
                        items: {
                            '$ref': items_ref
